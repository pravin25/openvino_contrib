...
{
    const float EPSILON = 1e-9; // Helps match CPU "hard zeros"
    ...
    for(int k = 0; k < 3; k++) {
        ...
        //                    [-s  c] [v_q]
        float next_p = jl.c * v_p + jl.s * v_q;
        float next_q = -jl.s * v_p + jl.c * v_q;
...
__kernel void ov_custom_svd_u(
        __global const INPUT0_TYPE* input,   // (B,3,3)
        __global OUTPUT0_TYPE* U)             // (B,3,3)
{
    ...
    //float considerAsZero = 1.17549435e-38f;        // FLT_MIN
        ...
        for (int p = 1; p < 3; ++p)
        {
            for (int q = 0; q < p; ++q)
            {
                ...
                if (fabs(W[p][q]) > threshold || fabs(W[q][p]) > threshold)
                {
                    ...
                    JacobiRotation jl_t = { jl.c, -jl.s };
                    accumulate_U(Ut, p, q, jl_t);
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        // if diagonal entry is negative → flip column i of U
        if(a < 0)
        {
            //printf("\n changing sign");
            for (int k = 0; k < 3; k++)
    ...
    // Store U
    for(int i=0;i<3;i++) for(int j=0;j<3;j++) U[base+i*3+j]=Ut[i][j];
}
